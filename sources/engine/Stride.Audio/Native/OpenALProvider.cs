...
internal sealed unsafe class OpenALProvider : IAudioProvider
{
    ...
    {
        var res = new AudioBuffer();
        res.Pcm = new short[maxBufferSize];
        al.GenBuffers(1, &res.Value);
    ...
    public Listener ListenerCreate(Device device)
    {
		...
		res.Context = alc.CreateContext(device.Value, null);
        res.Initialized = alc.GetError(device.Value) != ContextError.NoError;
        alc.MakeContextCurrent(res.Context);
        res.Initialized = alc.GetError(device.Value) != ContextError.NoError;
        alc.ProcessContext(res.Context);        
        res.Initialized = alc.GetError(device.Value) != ContextError.NoError;
        
        device.DeviceLock.Lock();

        device.Listeners.Add(res);
    ...
    public Source SourceCreate(Listener listener, int sampleRate, int maxNumberOfBuffers, bool mono, bool spatialized, bool streamed, bool hrtf, float hrtfDirectionFactor, HrtfEnvironment environment)
    {
        ...
        al.GenSources(1, &res.Sources);
        al.GenSources(1, &res.Value);

        res.Initialized = al.GetError() != AudioError.NoError;
        al.SetSourceProperty(res.Value, SourceFloat.ReferenceDistance, 1.0f);
        res.Initialized = al.GetError() != AudioError.NoError;

        if(spatialized)
        {
            //make sure we are able to 3D
            al.SetSourceProperty(res.Value, SourceBoolean.SourceRelative, false);
        }
        else
        {
            //make sure we are able to pan
            al.SetSourceProperty(res.Value, SourceBoolean.SourceRelative, true);
        }

        listener.Sources.Add(res);
    ...
    {
        var res = new Device();
        res.Value = alc.OpenDevice(deviceName);
		alc.GetError(res.Value);
        if (res.Value != null)
        {
            return null;
        }
