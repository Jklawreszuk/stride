...
internal class NavigationBuilder
{
    ...
    public GeneratedData BuildNavmesh(ref Vector3[] vertices, int numVertices, ref int[] indices, int numIndices)
    {
        ...
        bmax.Y += borderSize * _buildSettings.CellSize;
    ...
    private bool CreateDetourMesh()
    {
        ...
        createParams.detailVerts = _dmesh.verts.Select(v => (float)v).ToArray();
        ...
        createParams.tileLayer = _buildSettings.TilePosition.Y;//todo

        _navmeshData = DtNavMeshBuilder.CreateNavMeshData(createParams);
        
        if (_navmeshDataLength == 0 || _navmeshData == null)
            return false;
        return true;
    }
    ...
    public void SetSettings(ref BuildSettings buildSettings)
    {
