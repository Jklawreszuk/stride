...
namespace Stride.Core.Presentation.Themes
{
    ...
    public static class ImageThemingUtilities
    {
        ...
        public static Drawing TransformDrawing(Drawing drawing, IconTheme theme, bool checkLuminosity = true)
        {
        ...
        private static void TransformParts(Drawing drawing, IconTheme theme)
        {
            if (drawing is GeometryDrawing gd && gd.Brush is SolidColorBrush s)
            {
                var hsl = s.Color.ToHsl();
                var newL = TransformLuminosity(hsl, theme.BackgroundLuminosity);
                var newColor = new HslColor(hsl.H, hsl.S, newL, hsl.A).ToRgb();
                s.Color = newColor;
            }
            else if (drawing is DrawingGroup dg)
            {
                foreach (Drawing dr in dg.Children)
                {
