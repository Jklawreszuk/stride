...
internal class PVRTexture : IDisposable
{
    ...
    [DllImport("PVRTexLib", CallingConvention = CallingConvention.Cdecl), SuppressUnmanagedCodeSecurity]
    private static extern IntPtr PVRTexLib_GetTextureDataPtr(IntPtr texture, uint MIPLevel, uint arrayMember, uint faceNumber, uint ZSlice);
    ...
    public IntPtr GetDataPtr(uint uiMIPLevel = 0, uint uiArrayMember = 0, uint uiFaceNumber = 0, uint zSlice = 0)
    {
        return PVRTexLib_GetTextureDataPtr(texture, uiMIPLevel, uiArrayMember, uiFaceNumber, zSlice);
    }
