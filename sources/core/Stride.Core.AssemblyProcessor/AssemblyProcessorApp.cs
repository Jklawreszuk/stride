...
public class AssemblyProcessorApp
{
    ...
    {
        outputFile ??= inputFile ?? throw new ArgumentNullException(nameof(inputFile));
        ...
        try
        {
            try
            {
                ...
                var result = Run(ref assemblyDefinition, ref readWriteSymbols, out var modified, out var serializationHash);
    ...
    public bool Run(ref AssemblyDefinition assemblyDefinition, ref bool readWriteSymbols, out bool modified, out ObjectId? serializationHash)
    {
        ...
        try
        {
            ...
            {
                new AddReferenceProcessor(ReferencesToAdd)
            };
